...
int hdf5_nbi_read(hid_t f, int* n_inj_out, nbi_injector** inj_out) {
    ...
    hdf5_generate_qid_path("/nbi/nbi-XXXXXXXXXX", active, path);
    if(hdf5_find_group(f, path) == 0) {
        int n_inj;

        err = H5LTread_dataset_int(f, hdf5_generate_qid_path("/nbi/nbi-XXXXXXXXXX/ninj", active, path), &n_inj);
        ...
        for(int i = 0; i < n_inj; i++) {
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/id", i+1);
            err = H5LTread_dataset_int(f, hdf5_generate_qid_path(path, active, path), &(inj[i].id));

            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/nbeamlet", i+1);
            ...
            inj[i].beamlet_dirx = malloc(inj[i].n_beamlet * sizeof(real));
            inj[i].beamlet_diry = malloc(inj[i].n_beamlet * sizeof(real));
            inj[i].beamlet_dirz = malloc(inj[i].n_beamlet * sizeof(real));
            
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/beamletx", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].beamlet_x);
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/beamlety", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].beamlet_y);
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/beamletz", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].beamlet_z);
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/beamletdirx", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].beamlet_dirx);
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/beamletdiry", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].beamlet_diry);
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/beamletdirz", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].beamlet_dirz);

            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/power", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), &(inj[i].power));
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/energy", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), &(inj[i].energy));
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/efrac", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].efrac);
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/divergence", i+1);
            err = H5LTread_dataset_double(f, hdf5_generate_qid_path(path, active, path), inj[i].divergence);
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/anum", i+1);
            err = H5LTread_dataset_int(f, hdf5_generate_qid_path(path, active, path), &(inj[i].anum));
            sprintf(path, "/nbi/nbi-XXXXXXXXXX/inj%d/znum", i+1);
