...
a5err B_2DS_eval_psi_SIMD(int i, real psi[NSIMD], real r, real phi, real z, B_2DS_data* Bdata) {
    ...
    int interperr = 0; /* If error happened during interpolation */
    #if INTERP_SPL_EXPL
    interperr += interp2Dexpl_eval_B(&psi[i], &Bdata->psi, r, z);
    #else
...
a5err B_2DS_eval_B_SIMD(int i, real B[3][NSIMD], real r, real phi, real z, B_2DS_data* Bdata) {
    ...
    #if INTERP_SPL_EXPL
    interperr += interp2Dexpl_eval_B(&B[0][i], &Bdata->B_r, r, z);
    interperr += interp2Dexpl_eval_B(&B[1][i], &Bdata->B_phi, r, z);
    interperr += interp2Dexpl_eval_B(&B[2][i], &Bdata->B_z, r, z);
    #else
    interperr += interp2Dcomp_eval_B_SIMD(i, B[0], &Bdata->B_r, r, z);
    interperr += interp2Dcomp_eval_B_SIMD(i, B[1], &Bdata->B_phi, r, z);
    interperr += interp2Dcomp_eval_B_SIMD(i, B[2], &Bdata->B_z, r, z);
    #endif
...
a5err B_2DS_eval_B_dB_SIMD(int i, real B_dB[12][NSIMD], real r, real phi, real z, B_2DS_data* Bdata) {
    a5err err = 0;
    int interperr = 0; /* If error happened during interpolation */
    real B_dB_temp[6][NSIMD];
    
   
    #if INTERP_SPL_EXPL
    //interperr += interp2Dexpl_eval_dB(B_dB_temp, &Bdata->B_r, r, z);
    #else
    interperr += interp2Dcomp_eval_dB_SIMD(i, B_dB_temp, &Bdata->B_r, r, z);
    #endif

    B_dB[0][i] = B_dB_temp[0][i];
    B_dB[1][i] = B_dB_temp[1][i];
    B_dB[2][i] = 0;
    B_dB[3][i] = B_dB_temp[2][i];

    #if INTERP_SPL_EXPL
    //interperr += interp2Dexpl_eval_dB(B_dB_temp, &Bdata->B_phi, r, z);
    #else
    interperr += interp2Dcomp_eval_dB_SIMD(i, B_dB_temp, &Bdata->B_phi, r, z);
    #endif

    B_dB[4][i] = B_dB_temp[0][i];
    B_dB[5][i] = B_dB_temp[1][i];
    B_dB[6][i] = 0;
    B_dB[7][i] = B_dB_temp[2][i];

    #if INTERP_SPL_EXPL
    //interperr += interp2Dexpl_eval_dB(B_dB_temp, &Bdata->B_z, r, z);
    #else
    interperr += interp2Dcomp_eval_dB_SIMD(i, B_dB_temp, &Bdata->B_z, r, z);
    #endif

    B_dB[8][i] = B_dB_temp[0][i];
    B_dB[9][i] = B_dB_temp[1][i];
    B_dB[10][i] = 0;
    B_dB[11][i] = B_dB_temp[2][i];
