/**
 * @file B_3DST.c
 * @brief Time dependent 3D magnetic field with tricubic spline interpolation
 *
 ...
 * - B_3DST_offload_data.n_phi
 * - B_3DST_offload_data.phi_min
 * - B_3DST_offload_data.phi_max
 *
...
a5err B_3DST_eval_psi(real psi[1], real r, real phi, real z,
    ...
    if(interperr) {err = error_raise( ERR_INPUT_EVALUATION, __LINE__, EF_B_3DST );}
 ...
 * @return Non-zero a5err value if evaluation failed, zero otherwise
 */
a5err B_3DST_eval_B(real B[3], real r, real phi, real z, B_3DST_data* Bdata) {
    a5err err = 0;
    int interperr = 0; /* If error happened during interpolation */
#if INTERP_SPL_EXPL
    ...
    if(!err && check) {err = error_raise( ERR_INPUT_UNPHYSICAL, __LINE__, EF_B_3DST );}
 ...
 * @return Non-zero a5err value if evaluation failed, zero otherwise
 */
a5err B_3DST_eval_B_dB(real B_dB[], real r, real phi, real z,
                      B_3DST_data* Bdata) {
    a5err err = 0;
    int interperr = 0; /* If error happened during interpolation */
    real B_dB_temp[10];
