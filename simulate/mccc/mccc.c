...
void mccc_update_fo(particle_simd_fo* p, B_field_data* Bdata, plasma_data* pdata, real* coldata, 
    ...
    for(i = 0; i < NSIMD; i++) {
        if(p->running[i]) {
            ...
            const real* q_species = plasma_get_species_charge(pdata);
            const real* m_species = plasma_get_species_mass(pdata);

            plasma_eval_densandtemp(dens, temp, p->rho[i], p->r[i], p->phi[i],
...
void mccc_step_fo_fixed(particle_simd_fo* p, B_field_data* Bdata, plasma_data* pdata, random_data* rdata, real* coldata, real* h){
    ...
    for(i = 0; i < NSIMD; i++) {
        if(p->running[i]) {
            ...
            real dens[MAX_SPECIES];
            if(!errflag) {
                errflag = plasma_eval_densandtemp(dens, temp, p->rho[i],
                                                  p->r[i], p->phi[i], p->z[i],
                                                  p->time[i], pdata);
            }
            for(int j = 0; j < n_species; j++) {
