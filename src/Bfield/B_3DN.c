/**
 * @file B_3DN.c
 * @brief 3D magnetic field with neural network interpolation
 *
 * This module represents a magnetic field where data is given in \f$R\phi z\f$-
 * grid from which it is interpolated using a neural network.
 *
 * TODO: Check if description below is still accurate
 * 
...
int B_3DN_init(B_3DN_data* data,
    ...
    err = B_3DN_eval_psi(psival, data->axis_r, 0, data->axis_z, data);
    err = B_3DN_eval_B(Bval, data->axis_r, 0, data->axis_z, data);
    if(err) {
 ...
 */
a5err B_3DN_eval_psi(real* psi, real r, real phi, real z,
                   B_3DN_data* Bdata) {
    a5err err = 0;
...
a5err B_3DN_eval_rho_drho(real rho_drho[4], real r, real phi, real z,
    ...
    if( (psi_dpsi[0] - Bdata->psi0) / delta < 0 ) {
         return error_raise( ERR_INPUT_UNPHYSICAL, __LINE__, EF_B_3DN );
    }
...
a5err B_3DN_eval_B(real B[3], real r, real phi, real z, B_3DN_data* Bdata) {
    ...
    if(!err && check) {
        err = error_raise( ERR_INPUT_UNPHYSICAL, __LINE__, EF_B_3DN );
    }
