/**
 * @file rf_fields_fo.c
 * @brief 3D electromagnetic fields from TORIC RF wave evaluation.
 *
...
a5err RF3D_fields_init_from_file(RF3D_fields* rffield_data, hid_t f, char* qid){
    ...
    int n_r, n_z, n_phi; // Number of grid points in r, z and phi directions
    ...
    if( hdf5_read_int(RFPATH "nz", &n_z,
                      f, qid, __FILE__, __LINE__) ) {return 1;}
    if( hdf5_read_int(RFPATH "nphi", &n_phi,
                      f, qid, __FILE__, __LINE__) ) {return 1;}
    ...
    /* Initialize the RF3D_fields struct */
    err = RF3D_fields_init(rffield_data, rmin, rmax, n_r, zmin, zmax, n_z, n_phi, n_tor, omega, \
...
int RF3D_fields_init(RF3D_fields* rffield_data, real rmin, real rmax, int nr, \
    ...
    for(int i = 0; i < 12; i++){
        err += interp3Dcomp_setup(rffield_data->introbj[i], \
                                  data[i], nr, nz, nphi, \
                                  NATURALBC, NATURALBC, PERIODICBC,\
                                  rmin, rmax, zmin, zmax, 0, 2*M_PI);
        if(err) break;
    }
...
void RF3D_fields_offload(RF3D_fields* rffield_data){
    ...
    int nsize = rffield_data->Er_real.n_x * rffield_data->Er_real.n_y * rffield_data->Er_real.n_z;
    // Offload the data to the accelerator.
    GPU_MAP_TO_DEVICE(rffield_data->Er_real, rffield_data->Er_real.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Er_imag, rffield_data->Er_imag.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Ez_real, rffield_data->Ez_real.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Ez_imag, rffield_data->Ez_imag.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Ephi_real, rffield_data->Ephi_real.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Ephi_imag, rffield_data->Ephi_imag.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Br_real, rffield_data->Br_real.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Br_imag, rffield_data->Br_imag.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Bz_real, rffield_data->Bz_real.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Bz_imag, rffield_data->Bz_imag.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Bphi_real, rffield_data->Bphi_real.c[0:nsize*NSIZE_COMP3D], \
                      rffield_data->Bphi_imag, rffield_data->Bphi_imag.c[0:nsize*NSIZE_COMP3D],
...
a5err RF3D_field_eval(real E[3], real B[3], real r, real phi,\
    ...
    interperr += interp3Dcomp_eval_f(&interpolated[0],  &rffield_data->Er_real,   r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[1],  &rffield_data->Er_imag,   r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[2],  &rffield_data->Ephi_real, r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[3],  &rffield_data->Ephi_imag, r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[4],  &rffield_data->Ez_real,   r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[5],  &rffield_data->Ez_imag,   r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[6],  &rffield_data->Br_real,   r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[7],  &rffield_data->Br_imag,   r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[8],  &rffield_data->Bphi_real, r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[9],  &rffield_data->Bphi_imag, r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[10], &rffield_data->Bz_real,   r, z, phi);
    interperr += interp3Dcomp_eval_f(&interpolated[11], &rffield_data->Bz_imag,   r, z, phi);
    ...
    real phase = rffield_data->omega * t;
