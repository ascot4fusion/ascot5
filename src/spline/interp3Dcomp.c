...
void interp3Dcomp_init_spline(interp3D_data* str, real* c,
    ...
    str->xg2_over6  = (str->x_grid * str->x_grid) / 6.0;
    str->yg2_over6  = (str->y_grid * str->y_grid) / 6.0;
    str->zg2_over6  = (str->z_grid * str->z_grid) / 6.0;

    str->xg_over6  = str->x_grid / 6.0;
    str->yg_over6  = str->y_grid / 6.0;
    str->zg_over6  = str->z_grid / 6.0;

    str->y_stride8  = str->n_x * 8;
    str->z_stride8  = str->n_y * str->n_x * 8;
}
...
a5err interp3Dcomp_eval_df(real* f_df, interp3D_data* str,
    ...
    const real zg2_over6 = str->zg2_over6;

    const real xg_over6 = str->xg_over6;
    const real yg_over6 = str->yg_over6;
    const real zg_over6 = str->zg_over6;

    real xg     = str->x_grid;
    real xg2    = xg*xg;
    real xgi    = str->x_inv_grid;
    /* Index for x variable. The -1 needed at exactly grid end. */
    int i_x     = (x - str->x_min) * xgi - 1*(x==str->x_max);
    /* Normalized x coordinate in current cell */
    real dx     = ( x - (str->x_min + i_x*str->x_grid) ) * xgi;
    /* Helper variables */
    real dx3    = dx*dx*dx - dx;
    ...
    real yg     = str->y_grid;
    real yg2    = yg*yg;
    real ygi    = str->y_inv_grid;
    /* Index for y variable. The -1 needed at exactly grid end. */
    int i_y     = (y - str->y_min) * ygi - 1*(y==str->y_max);
    /* Normalized y coordinate in current cell */
    real dy     = ( y - (str->y_min + i_y*str->y_grid) ) * ygi;
    /* Helper variables */
    real dy3    = dy*dy*dy-dy;
    ...
    real zg     = str->z_grid;
    real zg2    = zg*zg;
    real zgi    = str->z_inv_grid;
    /* Index for z variable. The -1 needed at exactly grid end. */
    int i_z     = (z - str->z_min) * zgi - 1*(z==str->z_max);
    /* Normalized z coordinate in current cell */
    real dz     = ( z - (str->z_min + i_z*str->z_grid) ) * zgi;
    /* Helper variables */
    real dz3    = dz*dz*dz - dz;
    ...
    real x, real y, real z)
{
    ...
    /* 3) indices via inv_grid */
    real fx = (x - str->x_min) * str->x_inv_grid;
    real fy = (y - str->y_min) * str->y_inv_grid;
    real fz = (z - str->z_min) * str->z_inv_grid;

    int i_x = (int)floor(fx);
    int i_y = (int)floor(fy);
    int i_z = (int)floor(fz);

    /* gérer pile au max (évite -1*(x==x_max) et divergence) */
    if (x >= str->x_max) i_x = str->n_x - 1;
    if (y >= str->y_max) i_y = str->n_y - 1;
    if (z >= str->z_max) i_z = str->n_z - 1;
    ...
    /* grids */
    const real xg  = str->x_grid;
    const real yg  = str->y_grid;
    const real zg  = str->z_grid;

    const real xg2_over6 = str->xg2_over6; /* xg*xg/6 */
    const real yg2_over6 = str->yg2_over6;
    const real zg2_over6 = str->zg2_over6;

    const real xgi = str->x_inv_grid; /* 1/xg */
    const real ygi = str->y_inv_grid;
    const real zgi = str->z_inv_grid;

    /* carrés utiles */
    const real xg2 = xg * xg;
    const real yg2 = yg * yg;
    const real zg2 = zg * zg;

    /* 6) jumps + périodique au bord */
    int x1 = 8;
    int y1 = str->y_stride8;  /* n_x*8 */
    int z1 = str->z_stride8;  /* n_y*n_x*8 */

    if (str->bc_x == PERIODICBC && i_x == str->n_x - 1) x1 = -(str->n_x - 1) * x1;
    if (str->bc_y == PERIODICBC && i_y == str->n_y - 1) y1 = -(str->n_y - 1) * y1;
    if (str->bc_z == PERIODICBC && i_z == str->n_z - 1) z1 = -(str->n_z - 1) * z1;

    /* 7) base index (comme ton code original) */
    int n = i_z * (str->n_y * str->n_x * 8) + i_y * (str->n_x * 8) + i_x * 8;
    ...
    /* ---- TES FORMULES, inchangées, avec remplacements de constantes ---- */
