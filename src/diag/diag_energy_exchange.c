...
void diag_energy_exchange_update_gc(diag_energy_exchange_data* data, 
    ...
    for(int i = 0; i < NSIMD; i++) {
        ...
        real rhopar_f = p_f->ppar[i] / (p_f->mass[i] * sqrt(Babs2_f));
        real rhopar_i = p_i->ppar[i] / (p_i->mass[i] * sqrt(Babs2_i));
        ...
        for (int j = 0; j < data->n_modes; j++) {
            ...
            real dE_f = (rhopar_f * alpha * Babs2_f / mass - charge * phipot) * omega;
            real dE_i = (rhopar_i * alpha * Babs2_i / mass - charge * phipot) * omega;
            data->dEnergy[j * data->nprt + index] += 0.5 * (dE_f + dE_i) * dt;

            // 2. Computing the source term S1.
            data->S1[j * data->nprt + index] += 0.5 * (vA2_f * dE_f * w1 + vA2_i * dE_i * w0) * dt;

            // 3. Computing the source term S2.
            real dEdotv_f = (rhopar_f * alpha_dot * Babs2_f - \
                              vA2_f * phipot_dot) * omega*omega;
            real dEdotv_i = (rhopar_i * alpha_dot * Babs2_i - \
                             vA2_i * phipot_dot) * omega*omega;
            data->S2[j * data->nprt + index] += 0.5 * (vA2_f * dEdotv_f + \
                                                   vA2_i * dEdotv_i) * dt;
        }
    }
}
