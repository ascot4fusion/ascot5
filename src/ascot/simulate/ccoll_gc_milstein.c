 ...
 */
void mccc_gc_milstein(MarkerGuidingCenter* p, real* hin, real* hout, real tol,
                      mccc_wienarr* w, Bfield* bfield, Plasma* plasma,
    ...
    for(int i = 0; i < NSIMD; i++) {
        if(p->running[i]) {
            ...
            if(!errflag) {
                errflag = Bfield_eval_b_db(B_dB, Xout_rpz[0], Xout_rpz[1],
                                            Xout_rpz[2], p->time[i] + hin[i],
                                            bfield);
            }
            if(!errflag) {
                errflag = Bfield_eval_psi(psi, Xout_rpz[0], Xout_rpz[1],
                                           Xout_rpz[2], p->time[i] + hin[i],
                                           bfield);
            }
            if(!errflag) {
