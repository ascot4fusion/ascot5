...
void step_gc_cashkarp(particle_simd_gc* p, real* h, real* hnext, real tol,
    ...
    for(i = 0; i < NSIMD; i++) {
        if(p->running[i]) {
            ...
            if(!errflag) {
                errflag = B_field_eval_rho(rho, psi[0], bfield);
            }
            ...
            if(!errflag) {
                ...
                real axisrz[2];
                errflag = B_field_get_axis_rz(axisrz, bfield, p->phi[i]);
        ...
        if(p->running[i]) {
            ...
            if(!errflag) {
                errflag = mhd_eval(mhd_dmhd, yprev[0], yprev[1], yprev[2],
                                   t0, MHD_INCLUDE_ALL, boozer, mhd, bfield);
            }
            if(!errflag) {
                step_gceom_mhd(
