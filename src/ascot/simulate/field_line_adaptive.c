...
real simulate_fl_adaptive_inidt(sim_data* sim, particle_simd_ml* p, int i);
...
void simulate_fl_adaptive(particle_queue* pq, sim_data* sim) {
    ...
    int n_running = particle_cycle_ml(pq, &p, &sim->bfield, cycle);
    ...
    for(i = 0; i < NSIMD; i++) {
        if(cycle[i] > 0) {
            ...
            hin[i] = simulate_fl_adaptive_inidt(sim, &p, i);
        }
    }
    ...
    while(n_running > 0) {
        ...
        if(sim->params->enable_orbit_following) {
            ...
            if(sim->params->enable_mhd) {
                step_ml_cashkarp_mhd(&p, hin, hout, tol, &sim->bfield,
                                     sim->boozer, &sim->mhd);
            }
            else {
                step_ml_cashkarp(&p, hin, hout, tol, &sim->bfield);
            }
        ...
        n_running = particle_cycle_ml(pq, &p, &sim->bfield, cycle);
