...
void interp3Dcomp_init_spline(Spline3D* str, real* c,
    ...
    /* Initialize the Spline3D struct */
    str->n_x    = n_x;
    str->n_y    = n_y;
    str->n_z    = n_z;
    str->bc_x   = bc_x;
...
err_t interp3Dcomp_eval_f(real* f, Spline3D* str, real x, real y, real z) {
    ...
    size_t i_x   = (x - str->x_min) / str->x_grid - 1*(x==str->x_max);
    ...
    size_t i_y   = (y - str->y_min) / str->y_grid - 1*(y==str->y_max);
    ...
    size_t i_z   = (z - str->z_min) / str->z_grid - 1*(z==str->z_max);
    ...
    /**< Index jump to cell */
    size_t n  = i_z*str->n_y*str->n_x*8 + i_y*str->n_x*8 + i_x*8;
    size_t x1 = 8;                   /* Index jump one x forward */
    size_t y1 = str->n_x*8;          /* Index jump one y forward */
    size_t z1 = str->n_y*str->n_x*8; /* Index jump one z forward */
 ...
 * @return zero on success and one if (x,y,z) point is outside the grid.
 */
err_t interp3Dcomp_eval_df(real* f_df, Spline3D* str,
