...
void interp2Dcomp_init_spline(Spline2D* str, real* c,
    ...
    /* Initialize the Spline2D struct */
    str->n_x    = n_x;
    str->n_y    = n_y;
    str->bc_x   = bc_x;
    str->bc_y   = bc_y;
...
err_t interp2Dcomp_eval_f(real* f, Spline2D* str, real x, real y) {
    ...
    size_t n  = i_y*str->n_x*4+i_x*4; /* Index jump to cell       */
    size_t x1 = 4;                    /* Index jump one x forward */
    size_t y1 = str->n_x*4;           /* Index jump one y forward */
