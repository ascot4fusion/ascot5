...
void diag_transcoef_record(
    diag_transcoef_data* data, size_t index, size_t id, real rho, real r,
...
void diag_transcoef_process_and_clean(
    diag_transcoef_data* data, size_t index, size_t id, int navg);
...
void diag_transcoef_update_fo(diag_transcoef_data* data, Options* options,
    ...
    for(int i=0; i < p_f->n_mrk; i++) {
        ...
        diag_transcoef_record(
            ...
            options->record_rho_instead_of_r, options->margin);
    }
    ...
    for(int i=0; i < p_f->n_mrk; i++) {
        ...
        diag_transcoef_process_and_clean(
            ...
            options->number_of_points_to_average);
    }
}
...
void diag_transcoef_update_ml(diag_transcoef_data* data, Options* options,
    ...
    for(int i=0; i < NSIMD; i++) {
        ...
        diag_transcoef_process_and_clean(
            data, p_f->index[i], p_f->id[i], options->number_of_points_to_average);
    }
}
 ...
 */
void diag_transcoef_record(diag_transcoef_data* data, size_t index,
                           size_t id, real rho, real r, real pitchsign,
