...
void rfof_init(Rfof *rfof)
{
    ...
    __ascot5_icrh_routines_MOD_call_initev_excl_marker_stuff(
        xml_filename, &xml_filename_len_ptr, &(rfof->rfglobal),
...
void rfof_set_up(rfof_marker *rfof_mrk, Rfof *rfof)
{
#ifdef RFOF
...
void rfof_resonance_check_and_kick_gc(
    MarkerGuidingCenter *p, real *hin, real *hout, rfof_marker *rfof_mrk,
    ...
    for (int i = 0; i < NSIMD; i++)
    {
        if (p->id[i] > 0 && p->running[i])
        {
            ...
            __ascot5_icrh_routines_MOD_call_rf_kick(
                ...
                &(rfof_mrk->history_array[i]), &(rfof->rfglobal),
                &(rfof_mrk->diag_array[i]), &(rfof->rfof_input_params),
