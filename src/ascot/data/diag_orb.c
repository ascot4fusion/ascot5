 ...
 */
void diag_orb_init(diag_orb_data* data, Options* options,
                   size_t nmarkers) {

    size_t step = nmarkers*options->number_of_points_per_marker;
    data->id = (real*) calloc( step, sizeof(real) );
    if(options->poincare) {
        data->pncrid = (real*) calloc( step, sizeof(real) );
        data->pncrdi = (real*) calloc( step, sizeof(real) );
    }
    switch(options->simulation_mode) {
    ...
    data->mrk_pnt = (size_t*) malloc( nmarkers*sizeof(size_t) );
    data->mrk_recorded = (real*) malloc( nmarkers*sizeof(real) );

    memset(data->mrk_pnt, 0, nmarkers*sizeof(size_t));
 ...
 * @param data orbit diagnostics data struct
 */
void diag_orb_free(diag_orb_data* data, Options* options){
    free(data->mrk_pnt);
 ...
 *        current time-step
 */
void diag_orb_update_fo(diag_orb_data* data, Options* options,
    ...
    if(!options->poincare) {
        ...
        for(size_t i= 0; i < NSIMD; i++) {
            ...
            if(p_f->id[i] > 0) {
                ...
                size_t imrk   = p_f->index[i];
                size_t ipoint = data->mrk_pnt[imrk];
                size_t idx    = imrk * options->number_of_points_per_marker
                               + ipoint;

                /* If this is the first time-step, record marker position. */
                if( data->id[imrk * options->number_of_points_per_marker] == 0 ) {
                    ...
                    ipoint++;
                    if(ipoint == options->number_of_points_per_marker) {
                        ipoint = 0;
                    }
                ...
                real dt = data->mrk_recorded[imrk] + options->interval
                    - p_f->mileage[i];
                if( dt <= 0 || p_f->endcond[i] > 0 ) {
                    idx = imrk * options->number_of_points_per_marker + ipoint;
        ...
        for(size_t i= 0; i < NSIMD; i++) {
            ...
            if( p_f->id[i] > 0 && (p_f->mileage[i] != p_i->mileage[i]) ) {
                ...
                for(size_t j=0; j < options->ntoroidalplots; j++) {
                    k = diag_orb_check_plane_crossing(
                        p_f->phi[i], p_i->phi[i], options->toroidal_angles[j]);
                    if(k) {
                        real d = 1-k;
                        idx = imrk * options->number_of_points_per_marker + ipoint;
                        data->id[idx]     = (real)p_f->id[i];
                ...
                for(size_t j=0; j < options->npoloidalplots; j++) {
                    k = diag_orb_check_plane_crossing(
                        p_f->theta[i], p_i->theta[i], options->poloidal_angles[j]);
                    if(k) {
                        real d = 1-k;
                        idx = imrk * options->number_of_points_per_marker + ipoint;
                        data->id[idx]     = (real)p_f->id[i];
                        ...
                        data->B_phi[idx]  = k*p_f->B_phi[i]  + d*p_i->B_phi[i];
                        data->B_z[idx]    = k*p_f->B_z[i]    + d*p_i->B_z[i];
                        data->pncrid[idx] = j + options->ntoroidalplots;
                ...
                for(size_t j=0; j < options->nradialplots; j++) {
                    k = diag_orb_check_radial_crossing(
                        p_f->rho[i],p_i->rho[i], options->radial_distances[j]);
                    if(k) {
                        real d = k;
                        k = 1-d;
                        idx = imrk * options->number_of_points_per_marker + ipoint;
                        ...
                        data->B_phi[idx]  = k*p_f->B_phi[i]  + d*p_i->B_phi[i];
                        data->B_z[idx]    = k*p_f->B_z[i]    + d*p_i->B_z[i];
                        data->pncrid[idx] = j + options->ntoroidalplots + options->npoloidalplots;
 ...
 *        current time-step
 */
void diag_orb_update_gc(diag_orb_data* data, Options* options,
    ...
    if(!options->poincare) {
        ...
        for(size_t i= 0; i < NSIMD; i++) {
            ...
            if(p_f->id[i] > 0) {
                size_t imrk   = p_f->index[i];
                size_t ipoint = data->mrk_pnt[imrk];
                size_t idx    = imrk * options->number_of_points_per_marker + ipoint;
            ...
            if( p_f->id[i] > 0 && (p_f->mileage[i] != p_i->mileage[i]) ) {
                ...
                for(size_t j=0; j < options->nradialplots; j++) {
                    ...
                    if(k) {
                        ...
                        data->B_z[idx]    = k*p_f->B_z[i]    + d*p_i->B_z[i];
                        data->pncrid[idx] =
                                j + options->ntoroidalplots + options->npoloidalplots;
                        data->pncrdi[idx] = 1 - 2 * (p_f->rho[i] < p_i->rho[i]);
 ...
 *        current time-step
 */
void diag_orb_update_ml(diag_orb_data* data, Options* options,
