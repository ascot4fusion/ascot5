...
int BfieldAnalytical_init(
    BfieldAnalytical *bfield, size_t nripple, real bphi, real rmajor,
    real rminor, real psiscaling, real ripplescaling, real rippledamping,
    real axisrz[2], real psilimits[2], real coefficients[13])
{
...
err_t BfieldAnalytical_eval_psi(
...
err_t BfieldAnalytical_eval_psi_dpsi(
    real psi_dpsi[4], real r, real z, BfieldAnalytical *bfield)
{
    ...
    psi_dpsi[1] =
        (bfield->psiscaling / bfield->rmajor) *
    ...
    real b[3], real r, real phi, real z, BfieldAnalytical *bfield)
{
    ...
    const real *C = bfield->coefficients;
    ...
    b[1] *= bfield->nripple ? (1 + delta * cos(bfield->nripple * phi)) : 1.0;

    return 0;
}

err_t BfieldAnalytical_eval_b_db(
    real b_db[12], real r, real phi, real z, BfieldAnalytical *bfield)
{
    ...
    real r2 = r * r, r3 = r2 * r, r4 = r3 * r, r5 = r4 * r;
    real z2 = z * z, z3 = z2 * z, z4 = z3 * z, z5 = z4 * z;
    real R3 = bfield->rmajor * bfield->rmajor * bfield->rmajor;
    ...
    b_db[0] =
        ...
        C[10] * (3 * r4 - 12 * z2 * r2) +
        C[11] * (40 * z4 - 45 * r4 - 240 * z2 * r2 * logr + 60 * r4 * logr);
    b_db[0] *= -bfield->psiscaling / (r * bfield->rmajor * bfield->rmajor);

    b_db[3] = C[3] * (-16 * r * z) + C[4] * (-60 * r * z - 48 * r * logr * z) +
              C[5] * (-96 * r3 * z + 64 * r * z3) +
              C[6] * (-1600 * r * z3 - 960 * r * logr * z3 +
                      1440 * r3 * logr * z + 960 * r3 * z) +
              C[8] * (2 * r) + C[9] * (-6 * r * logr - 3 * r) +
              C[10] * (12 * r3 - 24 * z2 * r) +
              C[11] * (-120 * r3 - 480 * z2 * r * logr - 240 * z2 * r +
                       240 * r3 * logr);
    b_db[3] = -b_db[3] * bfield->psiscaling / (r * R3) -
              b_db[0] / (r * bfield->rmajor);
    b_db[4] = 0;
    b_db[5] = C[2] * (-2) + C[3] * (-8 * r2) +
              C[4] * (-18 * r2 - 24 * r2 * logr + 24 * z2) +
              C[5] * (-24 * r4 + 96 * r2 * z2) +
              C[6] * (240 * z4 - 1680 * r2 * z2 - 1440 * r2 * logr * z2 +
                      360 * r4 * logr + 150 * r4) +
              C[9] * (6 * z) + C[10] * (-24 * z * r2) +
    ...
    b_db[2] =
        ...
        C[11] * (-120 * z * r3 - 160 * z3 * r * logr - 80 * z3 * r +
                 240 * z * r3 * logr);
    b_db[2] *= bfield->psiscaling / (r * bfield->rmajor * bfield->rmajor);

    b_db[9] = (1 - C[12]) * (3 * r2 / 2) + C[12] * (1.5 + logr) + C[1] * (2) +
              C[2] * (2 * logr + 3) + C[3] * (12 * r2 - 8 * z2) +
              C[4] * (36 * r2 * logr + 21 * r2 - 54 * z2 - 24 * logr * z2) +
              C[5] * (30 * r4 - 144 * r2 * z2 + 16 * z4) +
              C[6] * (-640 * z4 - 240 * logr * z4 + 2160 * r2 * logr * z2 +
                      2160 * r2 * z2 - 450 * r4 * logr - 165 * r4) +
              C[8] * (2 * z) + C[9] * (-6 * z * logr - 9 * z) +
              C[10] * (36 * z * r2 - 8 * z3) +
              C[11] * (-120 * z * r2 - 160 * z3 * logr - 240 * z3 +
                       720 * z * r2 * logr);
    b_db[9] = b_db[9] * bfield->psiscaling / (r * R3) -
              b_db[2] / (r * bfield->rmajor);
    b_db[10] = 0;
    b_db[11] = (C[3] * (-16 * r * z) + C[5] * (-96 * r3 * z + 64 * r * z3) +
                C[4] * (-60 * r * z - 48 * r * logr * z) +
                C[6] * (-1600 * r * z3 - 960 * r * logr * z3 +
                        1440 * r3 * logr * z + 960 * r3 * z) +
                C[8] * (2 * r) + C[9] * (-6 * r * logr - 3 * r) +
                C[10] * (12 * r3 - 24 * z2 * r) +
                C[11] * (-120 * r3 - 480 * z2 * r * logr - 240 * z2 * r +
                         240 * r3 * logr));
    b_db[11] *= bfield->psiscaling / (r * R3);

    r *= bfield->rmajor;
    z *= bfield->rmajor;
    real radius = sqrt((r - bfield->rmajor) * (r - bfield->rmajor) + z * z);
    real theta = atan2(z, r - bfield->rmajor);
    real delta = bfield->ripplescaling * exp(-0.5 * theta * theta) *
                 pow(radius / bfield->rminor, bfield->rippledamping);
    real Bphi = b_db[1];
    real Bpert = Bphi * delta * cos(bfield->nripple * phi);
    real dBpertdR = Bpert * (((r - bfield->rmajor) / radius) *
                                 (bfield->rippledamping / radius) +
                             (z / (radius * radius)) * theta);
    real dBpertdz =
        Bpert * ((z / radius) * (bfield->rippledamping / radius) -
                 ((r - bfield->rmajor) / (radius * radius)) * theta);

    b_db[1] += bfield->nripple ? Bpert : 0.0;
    b_db[7] += bfield->nripple ? -Bphi * delta * bfield->nripple *
                                     sin(bfield->nripple * phi)
                               : 0.0;
    b_db[6] += bfield->nripple ? b_db[6] * Bpert / Bphi + dBpertdR : 0.0;
    b_db[8] += bfield->nripple ? dBpertdz : 0.0;
