    ...
    real *amplitude_nm, real *omega_nm, real *phase_nm, real *alpha, real *phi)
{
    ...
    for (int i = 0; i < nmode; i++)
    {
        ...
        err += interp2Dcomp_setup(
            &data->alpha_nm[i], &alpha[i * nrho * ntime], nrho, ntime,
            NATURALBC, NATURALBC, rhomin, rhomax, tmin, tmax);
        err += interp2Dcomp_setup(
            &data->phi_nm[i], &phi[i * nrho * ntime], nrho, ntime, NATURALBC,
            NATURALBC, rhomin, rhomax, tmin, tmax);
    }
    return err;
}
    ...
    Boozer *boozer, mhd_nonstat_data *mhd, Bfield *bfield)
{
    ...
    if (!err && isinside)
    {
        err = B_field_eval_rho(rho, ptz[0], bfield);
    }

    int iterations = mhd->n_modes;
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        interperr += interp2Dcomp_eval_df(a_da, &(mhd->alpha_nm[i]), rho[0], t);
        interperr +=
            interp2Dcomp_eval_df(phi_dphi, &(mhd->phi_nm[i]), rho[0], t);
        ...
        real mhdarg = mhd->nmode[i] * ptz[8] - mhd->mmode[i] * ptz[4] -
                      mhd->omega_nm[i] * t + mhd->phase_nm[i];
        real sinmhd = sin(mhdarg);
        real cosmhd = cos(mhdarg);

        /* Sum over modes to get alpha, phi */
        mhd_dmhd[0] += a_da[0] * mhd->amplitude_nm[i] * cosmhd;
        mhd_dmhd[5] += phi_dphi[0] * mhd->amplitude_nm[i] * cosmhd;

        /* Time derivatives */
        mhd_dmhd[1] +=
            a_da[0] * mhd->amplitude_nm[i] * mhd->omega_nm[i] * sinmhd +
            a_da[2] * mhd->amplitude_nm[i] * cosmhd;
        mhd_dmhd[6] +=
            phi_dphi[0] * mhd->amplitude_nm[i] * mhd->omega_nm[i] * sinmhd +
            phi_dphi[2] * mhd->amplitude_nm[i] * cosmhd;

        /* R component of gradients */
        mhd_dmhd[2] +=
            mhd->amplitude_nm[i] * (a_da[1] * ptz[1] * cosmhd +
                                    a_da[0] * mhd->mmode[i] * ptz[5] * sinmhd -
                                    a_da[0] * mhd->nmode[i] * ptz[9] * sinmhd);
        mhd_dmhd[7] += mhd->amplitude_nm[i] *
                       (phi_dphi[1] * ptz[1] * cosmhd +
                        phi_dphi[0] * mhd->mmode[i] * ptz[5] * sinmhd -
                        phi_dphi[0] * mhd->nmode[i] * ptz[9] * sinmhd);

        /* phi component of gradients */
        mhd_dmhd[3] += (1 / r) * mhd->amplitude_nm[i] *
                       (a_da[1] * ptz[2] * cosmhd +
                        a_da[0] * mhd->mmode[i] * ptz[6] * sinmhd -
                        a_da[0] * mhd->nmode[i] * ptz[10] * sinmhd);
        mhd_dmhd[8] += (1 / r) * mhd->amplitude_nm[i] *
        ...
        mhd_dmhd[4] +=
            mhd->amplitude_nm[i] * (a_da[1] * ptz[3] * cosmhd +
                                    a_da[0] * mhd->mmode[i] * ptz[7] * sinmhd -
                                    a_da[0] * mhd->nmode[i] * ptz[11] * sinmhd);
        mhd_dmhd[9] += mhd->amplitude_nm[i] *
...
a5err mhd_nonstat_perturbations(
    ...
    int includemode, Boozer *boozer, mhd_nonstat_data *mhd, Bfield *bfield)
{
    a5err err = 0;
    real mhd_dmhd[10];
    if (!err)
    {
        err = mhd_nonstat_eval(
    ...
    if (!err)
    {
        err = B_field_eval_B_dB(B_dB, r, phi, z, t, bfield);
    }
