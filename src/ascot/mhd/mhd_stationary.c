    ...
    real *alpha, real *phi)
{
    ...
    for (int i = 0; i < nmode; i++)
    {
        ...
        err += interp1Dcomp_setup(
            &data->alpha_nm[i], &alpha[i * nrho], nrho, NATURALBC, rhomin,
            rhomax);
        err += interp1Dcomp_setup(
            &data->phi_nm[i], &phi[i * nrho], nrho, NATURALBC, rhomin, rhomax);
    }
    return err;
}
 ...
 * @param data pointer to the data struct
 */
void mhd_stat_offload(mhd_stat_data *data) { SUPPRESS_UNUSED_WARNING(data); }
    ...
    Boozer *boozer, mhd_stat_data *mhd, Bfield *bfield)
{
    ...
    for (int i = 0; i < mhd->n_modes; i++)
    {
        ...
        interperr += interp1Dcomp_eval_df(a_da, &(mhd->alpha_nm[i]), rho[0]);
        interperr += interp1Dcomp_eval_df(phi_dphi, &(mhd->phi_nm[i]), rho[0]);
...
a5err mhd_stat_perturbations(
    ...
    int includemode, Boozer *boozer, mhd_stat_data *mhd, Bfield *bfield)
{
    a5err err = 0;
    real mhd_dmhd[10];
    if (!err)
