 ...
 */
int asigma_init(
    Atomic *atomic, int nreac, int *z1, int *a1, int *z2, int *a2,
    ...
    real *nmax, int *nT, real *Tmin, real *Tmax, real *sigma)
{
    ...
    atomic->sigma = (interp1D_data *)malloc(nreac * sizeof(interp1D_data));
    atomic->sigmav = (interp2D_data *)malloc(nreac * sizeof(interp2D_data));
    atomic->BMSsigmav = (interp3D_data *)malloc(nreac * sizeof(interp3D_data));
...
void asigma_free(Atomic* atomic) {
    for (int i_reac = 0; i_reac < atomic->N_reac; i_reac++)
    {
