...
/**
 * Declare routines or functions for target execution.
 */
#if defined(GPU) && defined(_OPENMP)
...
/**
 * End the target declaration for routines or function.
 */
#if defined(GPU) && defined(_OPENMP)
...
/**
 * Declare SIMD loop and specifies variables which are uniform.
 */
#if defined(GPU) && defined(_OPENMP)
...
/**
 * Declare target or SIMD routines.
 */
#if defined(GPU) && defined(_OPENMP)
...
/**
 * End declaring target or SIMD routines.
 */
#if defined(GPU) && defined(_OPENMP)
...
/**
 * Declare target or SIMD routines with uniform variables.
 */
#if defined(GPU) && defined(_OPENMP)
...
/**
 * End declare target or SIMD routine region (with uniform variables).
 */
#if defined(GPU) && defined(_OPENMP)
...
/**
 * Make a parallel region (CPU only).
 */
#if defined(GPU)
...
/**
 * Ensure that the following loop within a parallel region is executed by
...
/**
 * Hint at compiler that the following data is already present in the target
 * (OpenACC only).
 */
#if defined(GPU) && defined(_OPENACC)
...
#ifdef _OPENACC
/**
 * Number of gangs (OpenACC parallel execution units).
 */
#define NGANGS 1024

/**
 * Number of workers per gang (OpenACC).
 */
#define NWORKERS 8

/**
 * Number of vectors per worker (OpenACC).
 */
#define NVECTORS 32
#endif // _OPENACC
