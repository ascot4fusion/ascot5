...
void __ascot5_icrh_routines_MOD_call_rf_kick(double*time, double*dtin,
    int* myMPIprocID, prt_rfof* rfof_data_pack, void** cptr_marker,
...
void rfof_interface_initev_excl_marker_stuff(rfof_data* rfof_data) {
    ...
    __ascot5_icrh_routines_MOD_call_initev_excl_marker_stuff(xml_filename,
        &xml_filename_len_ptr, &(rfof_data->cptr_rfglobal),
        &(rfof_data->cptr_rfof_input_params));
#endif
};
 ...
 * @param cptr_mem Handle to corresponding Fortran resonance memory matrix
 *                 pointer
...
#ifdef RFOF
    __ascot5_icrh_routines_MOD_call_initialise_res_mem(cptr_mem,
        cptr_mem_shape_i, cptr_mem_shape_j, cptr_rfglobal,
        cptr_rfof_input_param);
#endif
};
...
void rfof_interface_initialise_diagnostics(void** cptr_rfglobal,
    void** cptr_diagno) {
#ifdef RFOF
    __ascot5_icrh_routines_MOD_call_initialise_diagnostics(cptr_rfglobal,
        cptr_diagno);
#endif
};
...
void rfof_interface_set_marker_pointers(void** cptr_marker, int* id,
    ...
    __ascot5_icrh_routines_MOD_call_set_marker_pointers(cptr_marker, &id,
        ...
        &pphicanonical, &vpar, &vperp, &gyrof, &vdriftRho, &acc,
        isOrbitTimeAccelerated, is_already_allocated);
#endif
};
...
void rfof_interface_do_rfof_stuff_gc(particle_simd_gc* ascot_marker, real* hin,
    ...
    for (int i=0; i<NSIMD;i++) {
        if (ascot_marker->id[i] > 0 && ascot_marker->running[i]) {
            ...
            /** @brief Canonical momentum conjugate to phi.
           ...
           /* Update the parallel momentum of ASCOT marker after RF kick      */
           ascot_marker->ppar[i] = ascot_marker->mass[i]*(v_par_old +
                                   rfof_data_pack.dvpar);

            // The other proposed methods for evaluating ppar:
            /*
            printf("\nCompute ppar using different input fields of RFOF marker"
            ...
            real ppar5 = B/(ascot_marker->r[i]*ascot_marker->B_phi[i])*
                (p_phi - ascot_marker->charge[i]*psi);
            printf("5. ppar from p_phi = %e\n", ppar5);

            // 6. Get ppar from gyrof and mu
            real ppar6 = ascot_marker->mass[i]*CONST_C*sqrt((
                ascot_marker->charge[i]*B/(ascot_marker->mass[i]*gyrof))*
                (ascot_marker->charge[i]*B/(ascot_marker->mass[i]*gyrof)) -
                2*ascot_marker->mu[i]*B/(ascot_marker->mass[i]*CONST_C2) - 1.0);
...
void rfof_interface_deallocate_res_mem(void** cptr_res_mem,
    int* cptr_mem_shape_i, int* cptr_mem_shape_j) {
#ifdef RFOF
    __ascot5_icrh_routines_MOD_call_deallocate_res_mem(cptr_res_mem,
        cptr_mem_shape_i, cptr_mem_shape_j);
#endif
};
