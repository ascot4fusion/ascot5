...
int PlasmaDynamic1D_init(
    PlasmaDynamic1D* plasma, int nrho, int ntime, int nion, int* anum, int* znum,
    real* rho, real* time, real* mass, real* charge, real* Te, real* Ti,
    real* ne, real* ni, real* vtor
) {
...
void PlasmaDynamic1D_offload(PlasmaDynamic1D* plasma) {
    ...
    GPU_MAP_TO_DEVICE(
        ...
        plasma->anum[0:plasma->nspecies-1], plasma->znum[0:plasma->nspecies-1], \
        plasma->rho[0:plasma->nrho], plasma->time[0:plasma->ntime], \
...
a5err PlasmaDynamic1D_eval_temp(
    ...
    a5err err = PlasmaDynamic1D_eval_densandtemp(temp_dens, temp_temp, rho, t,
                                            plasma);
...
a5err PlasmaDynamic1D_eval_densandtemp(
    ...
    if(rho < plasma->rho[0]) {
        ...
        for(int i = 0; i < plasma->nspecies; i++) {
            ...
            p11 = plasma->dens[i_time*plasma->nspecies*plasma->nrho
                                 + i*plasma->nrho
                                 + i_rho];
            p12 = plasma->dens[i_time*plasma->nspecies*plasma->nrho
                                 + i*plasma->nrho
                                 + i_rho + 1];
            p21 = plasma->dens[(i_time+1)*plasma->nspecies*plasma->nrho
                                 + i*plasma->nrho
                                 + i_rho];
            p22 = plasma->dens[(i_time+1)*plasma->nspecies*plasma->nrho
                                 + i*plasma->nrho
                                 + i_rho + 1];
            ...
            if(i < 2) {
                ...
                p11 = plasma->temp[i_time*2*plasma->nrho
                                     + i*plasma->nrho
                                     +i_rho];
                p12 = plasma->temp[i_time*2*plasma->nrho
                                     + i*plasma->nrho
                                     + i_rho + 1];
                p21 = plasma->temp[(i_time+1)*2*plasma->nrho
                                     + i*plasma->nrho
                                     + i_rho];
                p22 = plasma->temp[(i_time+1)*2*plasma->nrho
                                     + i*plasma->nrho
                                     + i_rho + 1];
