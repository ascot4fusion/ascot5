...
init_fun("BfieldSpline2D_free", ctypes.POINTER(Struct))


@Leaf.register
class BfieldSpline2D(InputVariant):
    ...
    def _stage(
        ...
        self._cdata = Struct()
        if LIBASCOT.BfieldSpline2D_init(
            ctypes.byref(self._cdata), rgrid.size, zgrid.size, rgrid[[0, -1]].v,
            zgrid[[0, -1]].v, axisrz.v, psilimits.v, psi.v, br.v, bz.v, bphi.v,
            ):
            self._cdata = None
    ...
    def unstage(self) -> None:
        ...
        LIBASCOT.BfieldSpline2D_free(ctypes.byref(self._cdata))
        self._cdata = None
...
class CreateMixin(TreeMixin):
    ...
    def create_bfieldspline2d(
        ...
        (:class:`.BfieldSpline3D`) makes the simulations *much* faster, so using this
        ...
        for abscissa in ["rgrid", "zgrid"]:
            utils.validate_abscissa(locals()[abscissa], abscissa)

        leaf = BfieldSpline2D(note=note)
        leaf._stage(
