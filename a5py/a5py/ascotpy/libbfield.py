...
class AscotpyBfield(Ascotpy):
    ...
    def evaluate(self, R, phi, z, t, quantity, grid=False,
        ...
        if quantity in ["rho", "psi"]:
            out = self.eval_bfield(R, phi, z, evalrho=True, evalpsi=True)[quantity]

        if quantity in ["br", "bphi", "bz", "brdr", "brdphi", "brdz", "bphidr",
                        "bphidphi", "bphidz", "bzdr", "bzdphi", "bzdz"]:
            out = self.eval_bfield(R, phi, z, evalb=True)[quantity]

        if quantity == "divergence":
            out = self.eval_bfield(R, phi, z, evalb=True)
