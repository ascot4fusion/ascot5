...
from a5py.engine.auxiliary import rhotheta2rz

from .template import InputTemplate

class PoincareMarker(InputTemplate):

    @physlib.parse_units(
            rhomin="1", rhomax="1", pol="deg", time="s", pitch="1", energy="eV",
            tor="deg",
            )
    def __init__(
            self,
            ...
            rhomin: float=0,
            rhomax: float=1,
            pol: utils.ArrayLike=0*unyt.deg,
            time: utils.ArrayLike=0*unyt.s,
            pitch: utils.ArrayLike=1.0,
            energy: utils.ArrayLike=1e3*unyt.eV,
            species: Optional[str]=None,
            tor: Optional[utils.ArrayLike]=None,
            ):
        """Generate markers at fixed intervals in radius.

        These markers can be used to populate magnetic field uniformly, which
        results in nice looking Poincaré-plots.

        By default, the generated markers are field lines. Physical particles
        can be generated by passing marker species.
        ...
        rho = np.linspace(rhomin, rhomax, nmrk)
        r, z = rhotheta2rz(ascot.data.bfield.active, rho=rho, theta=pol, phi=tor, time=time)

        if species is None:
            mrk = ascot.data.create_fieldlinemarker(
                r=r, z=z, phi=tor, direction=pitch, time=time, preview=True,
                )
            super().__init__(ascot, "fieldlinemarker", mrk.export())
        else:
            ...
            mrk["time"][:]   = time
            mrk["energy"][:] = energy
            super().__init__(ascot, "GuidingcenterMarker", mrk)


class PoincareOptions(InputTemplate):

    @physlib.parse_units(tor="deg", pol="deg", cpulim="s")
    def __init__(
        self,
        simmode: int=4,
        tor: utils.ArrayLike=0*unyt.deg,
        pol: utils.ArrayLike=0*unyt.deg,
        ntorb: int=500,
        nporb: int=500,
        both: bool=False,
        maxrho: bool=False,
        mhd: bool=False,
        cpulim: Optional[utils.ArrayLike]=None,
        ):
