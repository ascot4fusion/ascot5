...
class RunMixin(DistMixin):
    ...
    def getstate_markers(self, mrktype, ids=None, whichstate: str="end"):
        ...
        if mrktype == "prt":
            ...
            state = self.getstate(*qnt, mode="prt", state=whichstate, ids=ids)
            for i, q in enumerate(qnt):
                mrk[q] = state[i]
        elif mrktype == "gc":
            qnt = ["r", "phi", "z", "weight", "time", "ekin", "pitch", "zeta",
                   "mass", "charge", "anum", "znum"]
            state = self.getstate(*qnt, mode="gc", state=whichstate, ids=ids)
            for i, q in enumerate(qnt):
                if q == "ekin":
                    mrk["energy"] = state[i]
                else:
