...
def run():
    ...
    if True:
        ...
        o["ENABLE_COULOMB_COLLISIONS"] = 0*o["ENABLE_ORBIT_FOLLOWING"] + 0
    ...
    go    = ascot5.read_hdf5(fn[0],"results")["results"]["run-"+qid[0]]["orbits"]["fo"]
    qid,date = ascot5.get_qids(fn[1],"results")
    go2gc = ascot5.read_hdf5(fn[1],"results")["results"]["run-"+qid[0]]["orbits"]["gc"]
    qid,date = ascot5.get_qids(fn[2],"results")
    gc    = ascot5.read_hdf5(fn[2],"results")["results"]["run-"+qid[0]]["orbits"]["gc"]

    print(gc["theta"][-1], go2gc["theta"][-1])
